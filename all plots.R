...
ggplot(NULL, aes(x = samp_df[ , specific])) +
  ...
  geom_line(aes(x = mu_hat, y = (norm_likelihood_nv[ , specific] * number), color = "likelihood")) +
  geom_area(aes(x = mu_hat, y = (norm_likelihood_nv[ , specific] * number)), fill = "yellow", alpha = .4) +

  # prior uni
  geom_line(aes(x = mu_hat, y = (prior_dens * number), color = "prior_uni")) +
  geom_area(aes(x = mu_hat, y = (prior_dens * number)), fill = "orange", alpha = .4) +
